...
# Paths
base_path <- 'posterior_database/data/data-raw/covid19imperial'

## Reading all data
d <- readRDS(file.path(base_path, 'COVID-19-up-to-date.rds'))

## get IFR and population from same file
ifr.by.country <- read.csv(file.path(base_path, 'popt_ifr.csv'))
...
serial.interval = read.csv(file.path(base_path, 'serial_interval.csv'))
covariates = read.csv(file.path(base_path, 'interventions.csv'), stringsAsFactors = FALSE)
...
### ADD TO POSTERIOR DB
data_name <- "covid19imperial_v2"
data_keywords <- c("")
data_title <- "Data on infected and deaths in the multiple version covid19 pandemic"
data_description <-
"Data on deaths and infections in the covid19 pandemic for 14 European contries.
The data is access from the github repository with git hash 09615b6"
data_urls <- c("https://github.com/ImperialCollegeLondon/covid19model/")
data_references <- c("flaxman2020report")
...
# Create data object
dat <- pdb_data(stan_data, info = di)

# Write data to database
write_pdb(dat, pdb_local())
