...
## Licences
Most model code are using the BSD-3 licence. So is this repository in general. Although, some models and some data might have other open licences such as MIT. Each model has a ```licence``` element in the info JSON files that specifies the actual licence of the model. Some data might also have licences, then that is specified in a similar way.
