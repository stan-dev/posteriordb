...
Finally we can access reference posterior draws and information on how those were computed as follows.

```{r}
rpd <- reference_posterior_draws(po)
rpd

rpdi <- reference_posterior_draws_info(po)
rpdi
```
...
Similarly we can access reference posterior expectations and quantiles

```{r}
rpm <- reference_posterior_moments(po)
rpm

rpmi <- reference_posterior_moments_info(po)
rpmi

rpq <- reference_posterior_quantiles(po)
rpq

rpqi <- reference_posterior_quantiles_info(po)
rpqi
```
