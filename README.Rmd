...
To simplify the use of `posteriordb`, there are convenience functions both in Python and in R. To use R see the [posteriordb-r](./python/README.md) repsoitory and to use Python, see the [posteriordb-r](./python/README.md)
...
Models included in posteriordb might be updated over time. Although the models will only have the same name in posteriordb if the log density is the same (up to a normalizing constant). Otherwise a new model will be included in the database.
